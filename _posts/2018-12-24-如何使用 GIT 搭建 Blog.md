 ...
 * 解决办法：
   将远程 repository 中的代码可以通过如下命令进行代码合并，注：pull = fetch + merge(git fetch origin mater & get merge origin master)，使用命令 `git pull --rebase origin master`，意思是拉取远程的文件把本地的覆盖。
