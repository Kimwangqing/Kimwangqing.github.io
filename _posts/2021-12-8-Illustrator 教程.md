...
* 相同点：
	...
	* 复合路径和复合形状都类似与编组形式，但左键看到并不是编组
	* 复合路径和复合形状状态下双击单个图形，成为隔离模式，可移动或编辑，空白处双击变回复合模式
* 不同点
	* 复合路径与前面图形保持一致，复合图形与后面图形保持一致
	* 复合路径以差集的形式存在（颜色与差集形式相反），复合图形的选择都在路径查找器形状模式里（单机扩展后可以合并成一个形状）
	* 复合路径可以很简单地释放（还原）路径：
		*  对象 > 复合路径 > 释放
		*  左键 > 释放复合路径
	* 复合形状释放复杂（见上退出复合模式）
...
双击魔棒可参数调整选择对象：容差值最大选择范围越大
![](../pic/Illustrator 教程/魔棒工具参数.png)

#### 反向选择
选择对象 > 选择 > 反象 = 选择除了选择对象以外的所有对象
